//! Vendored from `p3-batch-stark` v0.5.3 `prover.rs`, with two changes:
//!
//! 1. The hardcoded `LogUpGadget` is replaced by [`crate::gadget::EvalGadget`]
//!    (the polynomial-evaluation argument).
//! 2. Permutation challenges come from
//!    [`crate::gadget::sample_eval_challenges`] — a single shared challenge
//!    `z` per proof — instead of per-lookup challenges, because all ring
//!    identities must be evaluated at the same point.
//!
//! Everything else (commitment schedule, quotient computation, openings) is
//! unchanged and reuses public items of `p3-batch-stark`.
...
use p3_lookup::lookup_traits::{Kind, LookupData, LookupGadget};
...
use p3_batch_stark::config::{
    Challenge, Domain, StarkGenericConfig as SGC, Val, observe_instance_binding,
};
use p3_batch_stark::proof::{
    BatchCommitments, BatchOpenedValues, BatchProof, OpenedValuesWithLookups,
};
use p3_batch_stark::prover::{StarkInstance, quotient_values};
use p3_batch_stark::symbolic::{get_log_num_quotient_chunks, get_symbolic_constraints};

use crate::gadget::{sample_eval_challenges, EvalGadget};
    ...
    SymbolicExpressionExt<Val<SC>, SC::Challenge>: Algebra<SC::Challenge>,
{
    ...
    let lookup_gadget = EvalGadget::new();
        ...
        .map(|(i, (air, _pv))| {
            ...
            let lq_chunks =
                info_span!("infer log of constraint degree", air_idx = i).in_scope(|| {
                    get_log_num_quotient_chunks::<Val<SC>, SC::Challenge, A, EvalGadget>(
                        air,
                        layout,
                        &all_lookups[i],
                        config.is_zk(),
    ...
    // Sample the shared evaluation challenge `z` (one per proof).
    let challenges_per_instance = sample_eval_challenges::<SC>(&mut challenger, &all_lookups);
    ...
    instances
        ...
        .for_each(|((i, inst), ext_domain)| {
            if !all_lookups[i].is_empty() {
                ...
                #[cfg(debug_assertions)]
                {
                    use crate::stark::check_constraints::check_constraints;
    ...
    // NOTE: the upstream LogUp multiset balance debug check (`check_lookups`)
    // is intentionally removed: evaluation arguments are not multiset
    // arguments, so the balance invariant does not apply.
    ...
    for (i, trace_domain) in trace_domains.iter().enumerate() {
        ...
        let perm_vals: Vec<SC::Challenge> = lookup_data[i]
            ...
            .map(|ld| ld.expected_cumulated)
            .collect();
        let q_values = quotient_values::<SC, A, _, EvalGadget>(
            airs[i],
            &pub_vals[i],
            sym_layout,
            *trace_domain,
