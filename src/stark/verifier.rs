//! Vendored from `p3-batch-stark` v0.5.3 `verifier.rs`, with the same two
//! changes as the prover (EvalGadget instead of LogUpGadget; one shared
//! evaluation challenge) plus one more: `verify_batch` returns the
//! per-instance challenges on success, so the wrapper can recompute the
//! public polynomials' evaluations at `z` and compare them against the
//! published values — that comparison is what binds the committed trace to
...
/// Verifies a batch proof. On success returns the per-instance permutation
/// challenges (for [`EvalGadget`]: the shared evaluation point `z`,
/// replicated per lookup).
#[instrument(skip_all)]
...
) -> Result<Vec<Vec<SC::Challenge>>, VerificationError<PcsError<SC>>>
where
    SC: SGC,
    ...
    Challenge<SC>: BasedVectorSpace<Val<SC>>,
{
    ...
    // Sample the shared evaluation challenge `z` (must mirror the prover).
    ...
    for (i, air) in airs.iter().enumerate() {
        ...
        verify_constraints_with_lookups::<SC, A, EvalGadget, PcsError<SC>>(
            air,
            &verifier_data,
            &lookup_gadget,
        )
